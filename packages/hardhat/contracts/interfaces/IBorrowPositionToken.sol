...
//    function updateSnapshot(uint256 tokenId, uint newDebt) external;
    ...
    function getBorrow(uint tokenId) external returns (DataTypes.BorrowData memory);
    function getInterest(uint tokenId) external view returns (uint256);
    function partialRepayBorrow(uint256 tokenId, uint256 repaymentAmount, uint lastSnapshotDate, address sender) external returns(uint principalReduction);
