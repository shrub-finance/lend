...
// Tasks
task("initializePools", "Create pools for an initial environment").setAction(
  async (taskArgs, env) => {
...
task("testLendingPlatform", "Setup an environment for development").setAction(
  async (taskArgs, env) => {
    ...
    await env.run("distributeUsdc", { to: account1, amount: 1000000 });
    ...
    await env.run("provideLiquidity", {
      usdcAmount: 1000000,
      timestamp: toEthDate(twelveMonth),
      account: account1,
    }); // 12 month
    await env.run("borrow", {
      account: account2,
      timestamp: toEthDate(twelveMonth),
      borrowAmount: 100,
      collateralAmount: 1,
    ...
    await env.run("borrow", {
      account: account3,
      timestamp: toEthDate(threeMonth),
      borrowAmount: 22,
...
task("testExtendBorrow", "testing extend borrow").setAction(
  async (taskArgs, env) => {
    const { ethers, deployments, getNamedAccounts } = env;
    const { deployer, account1, account2, account3 } = await getNamedAccounts();
    const { oneMonth, threeMonth, sixMonth, twelveMonth } = getPlatformDates();
    ...
    await env.run("borrow", {
      account: account2,
      timestamp: toEthDate(oneMonth),
      borrowAmount: 2,
      collateralAmount: 0.001739,
      ltv: 50,
    });
...
task("testLendingPlatform2", "Setup an environment for development").setAction(
  async (taskArgs, env) => {
    const { ethers, deployments, getNamedAccounts } = env;
    const { deployer, account1, account2, account3 } = await getNamedAccounts();
    await env.run("enableAutomine", {});
    ...
    await env.run("borrow", {
      account: account1,
      timestamp: aug2025,
      borrowAmount: 111.123456,
      collateralAmount: 0.52345,
      ltv: 33,
    });
...
task("testLendingPlatform3", "Setup an environment for development").setAction(
  async (taskArgs, env) => {
    await env.run("enableAutomine", {});
    await partA(env);
    await partB(env);
...
task("testWithdraw", "setup account1 and account2 for withdraw").setAction(
  async (taskArgs, env) => {
    await env.run("enableAutomine", {});
    await partA(env);
    await partB(env);
...
async function partB(env: HardhatRuntimeEnvironment) {
  const { getNamedAccounts } = env;
  const { deployer, account1, account2, account3, account4, shrubTreasury } =
...
async function partD(env: HardhatRuntimeEnvironment) {
  ...
  const { deployer, account1, account2, account3, account4, shrubTreasury } =
    await getNamedAccounts();
  await env.run("partialRepayBorrow", {
    account: account3,
    tokenId: 0,
    repaymentAmount: 50,
  });
  await env.run("extendBorrow", {
    account: account4,
    tokenId: 1,
    newTimestamp: jan2027,
    ltv: 80,
...
async function partD2(env: HardhatRuntimeEnvironment) {
  ...
  const { deployer, account1, account2, account3, account4, shrubTreasury } =
    await getNamedAccounts();
  await env.run("extendDeposit", {
    account: account2,
    currentTimestamp: may2026,
    newTimestamp: aug2026,
  });
  await env.run("takeSnapshot", { account: deployer });
}
