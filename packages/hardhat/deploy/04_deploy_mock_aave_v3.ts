...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  // Set as the owner of aETH
    ...
    console.log(`Transferring ownership of AETH (${aETHDeployment.address}) to mockAaveV3 (${deployResult.address}) (tx: ${tx.hash})`);
    const receipt = await tx.wait();
    console.log(`Trasnferring ownership complete with a cost of ${receipt?.gasUsed} gas`);
};
export default func;
