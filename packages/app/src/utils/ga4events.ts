...
function gaEventFactory(action: string, category: string, label?: string, value?: number) {
  const eventObject: {
    category: string;
    action: string;
    label?: string;
...
export const ga4events = {
  ...
  depositInterest: (interestAmount: number) => gaEventFactory('Deposit_Interest', select, custom, interestAmount),
