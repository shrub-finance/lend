...
export const Button: FC<ButtonProps> = ({
  ...
  const baseButtonClassList =
    "rounded-full leading-[24px] transition-all duration-[300ms] ease-in-out";
  const primaryButtonClassList =
    "text-white bg-shrub-green-500 disabled:bg-shrub-grey-50 disabled:border-shrub-grey-100";
  const primaryButtonHover = "hover:!bg-shrub-green-900";

  const secondaryButtonClassList = "text-shrub-grey-700 bg-white border";
  const secondaryButtonHover =
    "hover:bg-shrub-grey-light hover:border-shrub-grey-50";

  const infoButtonClassList =
    "text-shrub-grey-700 bg-white border border-shrub-grey-50";
  const infoButtonHover = "hover:bg-shrub-green-900 hover:text-white";
