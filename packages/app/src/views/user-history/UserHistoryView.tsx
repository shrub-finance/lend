...
export const UserHistoryView: React.FC<UserHistoryViewProps> = ({}) => {
  ...
  function getNotes(userLog) {
    ...
    const tokenAmount = tokenAmountUnparsed ? ethers.BigNumber.from(tokenAmountUnparsed) : Zero;
    const principal = principalUnparsed ? ethers.BigNumber.from(principalUnparsed) : Zero;
    const interest = interestUnparsed ? ethers.BigNumber.from(interestUnparsed) : Zero;
    const collateral = collateralUnparsed ? ethers.BigNumber.from(collateralUnparsed) : Zero;
    const ethYield = ethYieldUnparsed ? ethers.BigNumber.from(ethYieldUnparsed) : Zero;
    const depositEndDate = deposit && deposit.lendingPool && fromEthDate(deposit.lendingPool.timestamp);
    const borrowEndDate = borrow && fromEthDate(borrow.timestamp);
    if (type === "Deposit") {
      return (<div className="flex flex-col">
        <span>Lockup Date {formatShortDate(depositEndDate)}</span>
        <span>Deposit Amount: {formatLargeUsdc(principal.add(interest))} USDC</span>
        <span>PST Tokens received: {formatWad(tokenAmount, 6)}</span>
      </div>)
    } else if (type === 'Withdraw') {
      return (<div className="flex flex-col">
        <span>Lockup Date {formatShortDate(depositEndDate)}</span>
        <span>Withdraw Amount: {formatLargeUsdc(principal.add(interest))} USDC</span>
      ...
      return (<div className="flex flex-col">
        ...
        <span>Total Payment: {formatLargeUsdc(principal.add(interest))} USDC</span>
      </div>)
    } else if (type === 'Borrow') {
      return (<div className="flex flex-col">
        <span>Repayment Due Date: {formatShortDate(borrowEndDate)}</span>
      ...
      <div className='relative overflow-x-auto border rounded-2xl'>
        <table className='w-full text-left text-shrub-grey  '>
          ...
          <tr>
            ...
            <th scope='col' className='px-6 py-3 text-shrub-grey font-medium'>txid</th>
            <th scope='col' className='px-6 py-3 text-shrub-grey font-medium'>Notes</th>
          </tr>
          </thead>
