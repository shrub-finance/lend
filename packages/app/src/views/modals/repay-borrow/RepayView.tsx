    ...
    return (
      <>
        <div className='flex items-center justify-between p-4 md:p-5 border-b rounded-t '>
          <h3 className='text-xl font-semibold text-shrub-grey-900 '>{partialPaymentRequested ? "Partial Repay" : "Repay"}</h3>
            ...
            <div className='flex flex-col'>
              <div className='card w-full text-left'>
                <div className='card-body'>
                    ...
                    {/*</div>*/}
                        <div className='form-control w-full'>
                            <label className='label'>
                              <span className='label-text text-shrub-blue'>Amount Due</span>
                            </label>
                            <div className='w-full text-xl font-semibold flex flex-row'>
                              <span
                                className='text-4xl font-medium text-left w-[500px]'>{formatLargeUsdc(borrow.debt)} USDC</span>
                              <Image src='/usdc-logo.svg' className='w-10 inline align-baseline' alt={'usdc logo'}
                                     width={10} height={10} />
                            </div>
                        </div>
                    <div className='form-control w-full mt-6'>
                      <label className='label'>
                          ...
                          <ul className='flex flex-row'>
                            <li className='mr-4'>
                              <input type='radio' id='fullPay' name='deposit-extension'
                                     value={formatLargeUsdc(borrow.debt)} className='hidden peer' required
                                     onChange={() => {
                                       setRepayAmount(formatLargeUsdc(borrow.debt));
                                       setIsFullPay(true);
                                     }} checked={isFullPay} />
                              <label htmlFor='fullPay'
                                ...
                                <div className='block'>
                                  <p className='w-full text-lg font-semibold'>Pay in Full</p>
                                </div>
                              </label>
                            </li>
                            <li className='mr-4'>
                              ...
                              <label htmlFor='partialPay'
                                     className='inline-flex items-center justify-center w-full px-4 py-3 text-shrub-grey bg-white border border-shrub-grey-light2 rounded-lg cursor-pointer peer-checked:shadow-shrub-thin peer-checked:border-shrub-green-50 peer-checked:bg-teal-50 peer-checked:text-shrub-green-500 hover:text-shrub-green hover:border-shrub-green select-none'>
                                <div className='block'>
                                  <p className='w-full text-lg font-semibold'>Pay Partially</p>
                                </div>
                              </label>
                            </li>
                          </ul>
                  ...
                  {!isFullPay &&
                    <PartialRepayView borrow={borrow} isFullPay={isFullPay} partialPaymentRequested={partialPaymentRequested} setPartialPaymentRequested={setPartialPaymentRequested}/>
                  }
